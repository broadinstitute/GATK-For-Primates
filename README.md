...
* Samples from multiple (but closely related) taxonomic units (_e.g_. species within a genus, or subspecies within a species) can be co-analysed together in this pipeline. This allows for downstream analyses that, while irrelevant to human studies, transpire to be common-use scenarios in wildlife comparative genomics (_e.g._ the assessment of hybridization, admixture, speciation, _etc._). This approach also facilitates the discovery and consistent genotyping of rare and low-frequency alleles across units, which may offer improved insights into complex evolutionary histories and traits.
